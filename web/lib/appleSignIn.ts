...
export function getAppleSignInErrorMessage(error: unknown): string {
  ...
  if (code === "APPLE_SIGN_IN_FAILED") {
    return "Apple sign-in could not finish right now. Please try again.";
  }
  if (code === "APPLE_SIGN_IN_INVALID_RESPONSE") {
    return "Apple returned an incomplete sign-in response. Please try again.";
  }
  if (code === "APPLE_SIGN_IN_NOT_HANDLED") {
    return "Apple sign-in was interrupted before it finished. Please try again.";
  }
