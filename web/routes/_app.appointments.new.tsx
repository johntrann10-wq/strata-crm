...
function buildSelectedAddonSuggestions(
  ...
  return services
    .filter((service) => !service.isAddon && selectedIds.has(service.id))
    .map((service) => {
      const linkedAddons = addonLinks
        .filter((link) => link.parentServiceId === service.id)
        .map((link) => services.find((candidate) => candidate.id === link.addonServiceId))
        .filter((candidate): candidate is ServiceCatalogRecord => Boolean(candidate))
        .filter((addon) => addon.active !== false && !selectedIds.has(addon.id));
              ...
              {servicesFetching ? (
                ...
                <div className="space-y-4">
                  ...
                  {selectedAddonSuggestions.length > 0 ? (
                    <div className="rounded-2xl border border-amber-200/80 bg-amber-50/55 p-3 sm:p-4">
                      <div className="flex flex-col gap-1">
                        <p className="text-sm font-semibold text-amber-950">Optional add-ons</p>
                        <p className="text-xs leading-5 text-amber-800/80">
                          Add linked services to this appointment without hunting through the full service list.
                        </p>
                      </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {suggestion.linkedAddons.map((addon) => (
                                <button
                                  ...
                                  className="inline-flex min-h-10 max-w-full items-center gap-2 rounded-full border border-amber-200 bg-white px-3 py-2 text-left text-xs font-semibold text-slate-800 shadow-sm transition-colors hover:bg-amber-50"
                                >
                  ...
                  {recommendedPackageTemplates.length + otherPackageTemplates.length > 0 && (
                    <Accordion type="single" collapsible defaultValue="packages" className="rounded-2xl border border-border/70 bg-card px-3 sm:px-4">
