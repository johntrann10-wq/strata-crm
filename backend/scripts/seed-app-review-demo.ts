...
async function main() {
  ...
  const mediaAssetsTableCheck = await db.execute(
    sql<{ rel: string | null }>`select to_regclass('public.media_assets') as rel`
  );
  const hasMediaAssetsTable = Boolean(mediaAssetsTableCheck.rows[0]?.rel);
  ...
  console.table({
    ...
    mediaAssets: hasMediaAssetsTable ? 3 : 0,
  });
