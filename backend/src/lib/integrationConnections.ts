...
export async function upsertUserIntegrationConnection(input: {
  businessId: string;
  userId: string;
  provider: IntegrationProvider;
  status: "pending" | "connected" | "action_required" | "error" | "disconnected";
...
export async function disconnectUserIntegrationConnection(
  businessId: string,
  userId: string,
  provider: IntegrationProvider
) {
  const connection = await getUserIntegrationConnection(businessId, userId, provider);
  if (!connection) return null;
  const [updated] = await db
    .update(integrationConnections)
    .set({
