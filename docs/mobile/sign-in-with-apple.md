...
1. On a physical iPhone, open the sign-in screen and confirm **Sign in with Apple** appears alongside email login.
...
8. Test a team invite with **Hide My Email** enabled. Confirm the invite still attaches to the correct existing account or invited user when Apple hides the real email.
...
11. Verify web Google auth and email/password auth still work after the Apple changes. The native iOS shell keeps Google hidden so review does not leave the app for browser-based auth.
